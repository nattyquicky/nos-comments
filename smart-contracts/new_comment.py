...
def Main(operation, args):
    ...
    if operation == "Comment":
        ...
        if serial_post:
            ...
            deserial_post[3].append(comment)
            tmp_post = Serialize(deserial_post)
            Put(context, post_id, tmp_post)
    ...
    if operation == "RemovePost":
        ...
        if serial_post:
            deserial_post = Deserialize(serial_post)
            if deserial_post[0] is user_hash:
    ...
    if operation == "GetAllPosts":
        ...
        serial_all_posts = Get(context, get_all_ids)
        if serial_all_posts:
